...
function SignIn() {
	...
	return (
		<main className='relative w-full h-screen overflow-hidden justify-center p-6 flex flex-col gap-12'>
			...
			></div>
			<div
				className='bg-blue-700 blur-xl w-70 rounded-full h-70 absolute top-1/3 -right-30 -z-999'
				style={{
					animation: 'pulseSoft 4s ease-in-out infinite',
				}}
			></div>
			<div
				className='bg-blue-400 blur-xl w-60 rounded-full h-60 absolute top-1/3 -right-30 -z-980'
				style={{
					animation: 'pulseSoft 4s ease-in-out infinite',
				}}
			></div>
			<div
				className='bg-blue-700 blur-xl w-80 rounded-full h-80 absolute -bottom-35 left-1/2 -translate-x-1/2 -z-999'
				style={{
					animation: 'pulseSoft 5s ease-in-out infinite',
				}}
			></div>
			<div
				className='bg-blue-400 blur-xl w-70 rounded-full h-70 absolute -bottom-35 left-1/2 -translate-x-1/2 -z-980'
				style={{
					animation: 'pulseSoft 5s ease-in-out infinite',
				}}
			></div>
			<div className='flex flex-col gap-2'>
				<svg
					...
					className='icon icon-tabler icons-tabler-outline icon-tabler-user-plus stroke-blue-700'
				>
			...
			<form onSubmit={handleSubmit} className='flex flex-col gap-6 z-10'>
				...
				<div className='mt-6 flex flex-col gap-2'>
					<div className='flex gap-2 active:bg-gray-100/40 rounded-md px-2 py-1'>
						<NavLink to='/' className='font-semibold text-blue-600/80 text-sm underline decoration-blue-600/80 underline-offset-4'>
							Tienes una cuenta?
						</NavLink>
					</div>
