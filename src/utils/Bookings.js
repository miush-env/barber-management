...
export const cancelBooking = async (uid, setAppointments) => {
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(
        `Error al cancelar en la pagina de inicio (${res.status})`,
      );
    }
...
export const GetBookingsStatus = async (
  ...
  try {
    ...
    if (todayBookings === true) {
      URL += `&afterStart=${encodeURIComponent(startOfDay.toISOString())}&beforeEnd=${encodeURIComponent(endOfDay.toISOString())}&status=upcoming`;
    }
    if (status === "cancelled") {
